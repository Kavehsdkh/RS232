...
architecture Behavioral of RS_232_Tx is
	...
	signal	Data_Int			:	unsigned	(7 downto 0)				:=	(others=>'0');
	...
	begin
		...
		if rising_edge(Clock) then
			...
			if (Send_Int = '1' and Send_Prev = '0' and Busy_Int = '0') then
				...
				Busy_Int			<=		'1';
				Packet_Generate		<=		'1';
				Parity_Bit			<=		Data_Int(0) xor Data_Int(1) xor Data_Int(2) xor Data_Int(3) xor
												Data_Int(4) xor Data_Int(5) xor Data_Int(6) xor Data_Int(7);
			...
			if (Packet_Generate = '1') then
				...
				Start_Sending		<=		'1';
				Full_Packet			<=		'1'&'1'&Parity_Bit&Data_Int&'0';
			...
			if (Start_Sending = '1') then
				...
				Serial_Int			<=		Full_Packet(to_integer(Data_Bit_Count));
